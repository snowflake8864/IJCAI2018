...
def item_unique(data):
    group_city = data[['city_id', 'hour']]
    group_cityed = group_city.groupby(['city_id'])['hour'].mean().reset_index().rename(columns={'hour': 'mean_hour'})
    pd.merge(data, group_cityed, 'left', on='city_id')
    group_item = data[['item_city_id', 'hour']]
    group_itemed = group_item.groupby(['item_city_id'])['hour'].mean().reset_index().rename(columns={'hour': 'item_city_id'})
    pd.merge(data, group_itemed, 'left', on='item_city_id')
    return data
...
def submit(train, test, best_iter):
    ...
    pd_result = pd.DataFrame({'instance_id': df_test_b["instance_id"], 'predicted_score': answer})
...
if __name__ == '__main__':
    ...
    '''
    dir = '../data/'
    df = pd.read_table(dir + 'round2_train.txt', engine='python', sep=" ")
    # df_test_a = pd.read_table(dir + 'round1_ijcai_18_test_a_20180301.txt', engine='python', sep=" ")
    df_test_b = pd.read_table(dir + 'round2_ijcai_18_test_a_20180425.txt', engine='python', sep=" ")

    # df_test_a['is_trade'] = -1

    # 删除重复的 instance_id
    # df = df.drop_duplicates(['instance_id'])
    data = pd.concat([df, df_test_b])
    ...
    data = do_Trick3(data)
    # 线下测试
    # data = item_unique(data)

    train = data.loc[(data.time >= '2018-09-07 00:00:01') & (data.time <= '2018-09-07 09:59:59')]  # 18,19,20,21,22,23,24
    test = data.loc[(data.time >= '2018-09-07 10:00:00') & (data.time <= '2018-09-07 11:59:59')]  # 暂时先使用第24天作为验证集
    best_iter = xgbCV(train, test)
    # 线上提交
    '''
    train = data[(data.is_trade.notnull()) & (data.is_trade.values != -1)]
