...
#has_seen=pd.read_csv('/home/cbc/alimama-data/feat/data/has_seen.csv')
...
data_train=pd.read_csv('../data/data_train_ori.csv')
print(len(data_train))
#data_train['times']=data_train['times'].astype(str)
data_test=pd.read_csv('../data/data_test_ori.csv')
print(len(data_test))
...
def convert_data(data):
    ...
    print('user_like')
    #item_user_day_click=data.groupby(['item_id', 'day','user_id']).size().reset_index().rename(columns={0: 'item_user_day_click'})  
    ...
    #shop_user_day_click=data.groupby(['shop_id', 'day','user_id']).size().reset_index().rename(columns={0: 'shop_user_day_click'})  
    ...
    #brand_user_day_click=data.groupby(['item_brand_id', 'day','user_id']).size().reset_index().rename(columns={0: 'item_brand_user_day_click'})  
    ...
    #context_user_day_click=data.groupby(['context_id', 'day','user_id']).size().reset_index().rename(columns={0: 'context_user_day_click'})  
...
############      
    ...
    #print('user_like_merge...')
    #data = pd.merge(data, item_user_day_click, 'left',on=['item_id','day','user_id'])
    #data = pd.merge(data, item_user_day_hour_click, 'left',on=['item_id','day','hour','user_id'])
    #data = pd.merge(data, shop_user_day_click, 'left',on=['shop_id','day','user_id'])
    #data = pd.merge(data, shop_user_day_hour_click, 'left',on=['shop_id','day','hour','user_id'])
    #data = pd.merge(data, brand_user_day_click, 'left',on=['item_brand_id','day','user_id'])
    #data = pd.merge(data, brand_user_day_hour_click, 'left',on=['item_brand_id','day','hour','user_id'])
    #data = pd.merge(data, context_user_day_click, 'left',on=['context_id','day','user_id'])
    #data = pd.merge(data, context_user_day_hour_click, 'left',on=['context_id','day','hour','user_id'])
    return data
...
def online_submit(data_test,data_train):
    ...
    train_x.to_csv('../data/online_train_x_all_b.csv',index=False)
    train_y.to_csv('../data/online_train_y_all_b.csv',index=False)

    print('online_test')
    test_x=make_test_set_2(train_start_time,train_end_time,test_start_time,test_end_time,data_test,data_train)
    print(len(test_x))
    test_x.to_csv('../data/online_test_all_b.csv',index=False)
    ...
    xgb_train = xgb.train(params, dtrain, num_boost_round,evals=watchlist, verbose_eval=True,)
    ...
    sub_data.to_csv('all-xgb.txt',index=False,sep=' ')
